    ...
    <van-pull-refresh v-model="isLoading" @refresh="onRefresh">
      <van-list v-model="loading" :finished="finished" @load="onLoad" id="list">
        <van-cell
          ...
          :key="item.msg"
          class="msg"
          :class="[Boolean(item.userID == user['id']) ? 'text-right' : 'text-left']"
        >
          <div class="text">
            {{ item.msg }}
            <i>({{ item.time }})</i>
  ...
  methods: {
    ...
    initWebSocket() {
      ...
      this.websock = new WebSocket(
        `${this.path + this.$store.state.user.id}/${this.receiver.id}`,
      );
    ...
    websocketclose() {
      // 关闭
      console.log('断开连接:', this.user.id);
    },
    scrollBottom() {
      setTimeout(() => {
        const list = document.getElementById('list'); // 获取对象
